...
test("ApiV1IncomeEstimate all", async () => {
  const res = await client.v1.incomeEstimate.all();
  ...
  expect(res.entries[0]).toHaveProperty("estimate_no");
...
test("IncomeEstimate by id", async () => {
  const res = await client.v1.incomeEstimate.by({ id: 1 });
  expect(res).toBeDefined();
  ...
  expect(res).toHaveProperty("estimate_no");
  ...
  expect(res.line_items[1]).toHaveProperty("description");
  expect(res.line_items[1]).toHaveProperty("line_item_type");
  expect(res.line_items[1]).toHaveProperty("name");
  expect(res.line_items[1]).toHaveProperty("position_in_group");
  expect(res.line_items[1]).toHaveProperty("price");
  expect(res.line_items[1]).toHaveProperty("proposition");
  expect(res.line_items[1]).toHaveProperty("quantity");
  expect(res.line_items[1]).toHaveProperty("total_net");
  expect(res.line_items[1]).toHaveProperty("unit");
  expect(res.line_items[1]).toHaveProperty("vat_rate");
