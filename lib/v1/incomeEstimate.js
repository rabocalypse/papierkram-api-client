class IncomeEstimate {
  ...
  async by({ id }) {
    const response = await fetch(
      `https://${this.client.subdomain}.papierkram.de/api/v1/income/estimates/${id}`,
      {
        headers: {
          method: "GET",
  ...
  async all(page, pageSize, config = {}) {
    ...
    const response = await fetch(
      `https://${this.client.subdomain}.papierkram.de/api/v1/income/estimates`,
      {
        headers: {
          method: "GET",
          Accept: "application/json",
